...
const createStatusSummary = (data) => {
  ...
  const lastIndex = data.working.length - 1;
  const statItems = [
    { value: data.working[lastIndex], label: "status.working", color: "rgba(34, 197, 94, 1)" },
    { value: data.issues[lastIndex], label: "status.issues", color: "rgba(251, 191, 36, 1)" },
    { value: data.broken[lastIndex], label: "status.broken", color: "rgba(239, 68, 68, 1)" }
  ];
